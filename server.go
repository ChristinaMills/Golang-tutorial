...
// this is a function receiver for the above struct
func (h *coasterHandlers) get(w http.ResponseWriter, r *http.Request) {
	fmt.Println("in the function reciever")
	coasters := make([]Coaster, len(h.store))

	//Only need to lock when we read
	h.Lock()
	i := 0
...
func (h *coasterHandlers) getRandom(w http.ResponseWriter, r *http.Request) {
	coasters := make([]Coaster, len(h.store))
